...
const GalleryManagement = () => {
  ...
  const handleEditVideoClick = (galleryVideo) => {
    console.log("video", galleryVideo);
    setSelectedVideo(galleryVideo);
    setShowGalleryVideoModal(true);
    setModalMode("edit");
  };
  ...
  const confirmDeleteVideo = async (id, closeToast) => {
    ...
    try {
      ...
      await axios.delete(
        `https://ba-dastoor-backend.onrender.com/api/newGalleryVideo/new-delete-galleryVideo/${id}`
      );
      toast.update(toastId, {
        render: "Photos deleted successfully",
        type: "success",
