...
const AddGalleryVideosModal = ({
  ...
  useEffect(() => {
    if (mode === "edit" && selectedVideo) {
      ...
      const filledVideos = {
        primary: selectedVideo.primaryImage || null,
        sibling1: null,
        sibling2: null,
        sibling3: null,
        sibling4: null,
      ...
      selectedVideo.siblings?.forEach((img) => {
        if (img.slot) {
          filledVideos[img.slot] = img;
        }
      });
  ...
  const handleUpload = async (e) => {
    ...
    try {
      if (isEditMode && selectedVideo) {
        ...
        console.log("edit");
        const id = selectedVideo._id; // ✅ FIX
        ...
        // }
        const res = await axios.put(
          `https://ba-dastoor-backend.onrender.com/api/newGalleryVideo/new-edit-galleryVideo/${id}`,
          fd
        );
        toast.update(toastId, {
          render: "Videos updated successfully ✅",
          type: "success",
          isLoading: false,
          autoClose: 2000,
        });
        ...
        const res = await axios.post(
          `https://ba-dastoor-backend.onrender.com/api/newGalleryVideo/new-upload-galleryVideo`,
          fd
        );
        console.log(res);
        toast.update(toastId, {
          ...
          <div>
            ...
            <label className="block w-full h-48 border-2 border-dashed rounded-xl cursor-pointer relative overflow-hidden">
              ...
              <input
                type="file"
                hidden
                onChange={(e) => handleVideoChange(e, "primary")}
              />
            </label>
          </div>
          {/* SIBLING SLOTS */}
